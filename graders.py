...
def revenue_growth(state: dict) -> float:
    return _to_float(state.get("revenue_growth_pct", 0.0))


def user_growth(state: dict) -> float:
    return _to_float(state.get("active_user_growth_pct", 0.0))
...
def cost_reduction(state: dict) -> float:
    return _to_float(state.get("operational_cost_reduction_pct", 0.0))


def satisfaction_floor(state: dict) -> float:
    return _to_float(state.get("min_customer_satisfaction", 0.0))


def quality_floor(state: dict) -> float:
    return _to_float(state.get("min_product_quality", 0.0))


def normalized_reward(state: dict) -> float:
    return _to_float(state.get("episode_normalized_reward", 0.0))


def retention_proxy(state: dict) -> float:
    return _to_float(state.get("retained_user_ratio", 0.0))
